...
bool ProjectManager::New(const std::string& name, const std::string& path) {
    ...
    std::filesystem::copy("./template/Assets", projectManager->projectDir + "/Assets", std::filesystem::copy_options::update_existing | std::filesystem::copy_options::recursive);
    std::filesystem::copy("./template/ProjectData", projectManager->projectDir + "/ProjectData", std::filesystem::copy_options::update_existing | std::filesystem::copy_options::recursive);
    std::filesystem::copy("./template/CMakeLists.txt", projectManager->projectDir + "/CMakeLists.txt", std::filesystem::copy_options::update_existing);
...
void ProjectManager::Compile(){
    ...
    std::string command = "cmake -S " + projectManager->projectDir
                        + " -B " +  projectManager->projectDir + "/ProjectData/temp"
                        + " -DEDITOR_PATH:STRING=" + std::filesystem::current_path().string()
                        + " -DRL_BUILD_FLAGS:STRING=\"" + RL_BUILD_FLAGS + "\""
                        + " -DRL_BUILD_CONFIG_FLAGS:STRING=\"" + RL_BUILD_CONFIG_FLAGS + "\""
    ...
    std::filesystem::copy_file(projectManager->projectDir + "/ProjectData/out/Game.dll", "./Game.dll", std::filesystem::copy_options::overwrite_existing);
