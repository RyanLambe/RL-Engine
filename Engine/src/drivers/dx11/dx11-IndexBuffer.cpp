...
DX11IndexBuffer::DX11IndexBuffer(const std::vector<uint32_t>& data, bool dynamic)
{
    ...
    DX_LOG_ERROR(DX11Context::GetDevice()->CreateBuffer(&bufferDesc, &initData, &indexBuffer));
}
...
void DX11IndexBuffer::Set(const std::vector<uint32_t>& data)
{
	if (!isDynamic) {
        ...
        DX_LOG_ERROR(DX11Context::GetContext()->Map(indexBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource));
