...
DX11Shader::DX11Shader(const std::string& vertexShaderPath, const std::string& pixelShaderPath, const std::weak_ptr<DX11Context>& contextPtr) : contextPtr(contextPtr) {
    ...
    DX_LOG_ERROR(context->GetDevice()->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, &pixelShader));

    //vertex shader
    DX_LOG_ERROR(D3DReadFileToBlob(vertexPath.c_str(), &blob));
    DX_LOG_ERROR(context->GetDevice()->CreateVertexShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, &vertexShader));
    ...
    DX_LOG_ERROR(context->GetDevice()->CreateInputLayout(ied, sizeof(ied)/sizeof(D3D11_INPUT_ELEMENT_DESC), blob->GetBufferPointer(), blob->GetBufferSize(), &inputLayout));
}
