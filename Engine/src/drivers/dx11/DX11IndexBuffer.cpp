...
DX11IndexBuffer::DX11IndexBuffer(const std::vector<uint32_t>& data, bool dynamic, const std::weak_ptr<DX11Context>& contextPtr) : contextPtr(contextPtr)
{
	isDynamic = dynamic;
...
void DX11IndexBuffer::Set(const std::vector<uint32_t>& data)
{
	if (!isDynamic) {
        ...
        if(auto context = contextPtr.lock()){
            DX_LOG_ERROR(context->GetContext()->Map(indexBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource));
        }
        else{
            RL_THROW_EXCEPTION("Cannot access Context");
        }
