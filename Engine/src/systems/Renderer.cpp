...
namespace rl
{
    void Renderer::Start() {
        ...
        std::shared_ptr<Shader> shader = Shader::Create("VertexShader.cso", "PixelShader.cso", Application::GetGraphicsContextPtr());
        shader->Enable();

        glm::vec4 test = glm::vec4(0, 1, 0, 1);
        std::shared_ptr<UniformBuffer> ub = UniformBuffer::Create(sizeof(glm::vec4), ShaderType::PixelShader, 0, Application::GetGraphicsContextPtr());
    ...
    void Renderer::Update()
    {
        ...
        for(const auto& cam : Camera::GetAllComponents()){
            if (cam.GetRenderTarget() == nullptr) {
                RL_LOG_WARNING("Please set render destination via Camera.RenderToWindow or Camera.RenderToTarget.");
                continue;
            }
