                                                           ...
                                                           const std::weak_ptr<GraphicsContext> &context)
{
    switch (GraphicsContext::GetAPI())
    {
        ...
        case GraphicsAPI::Vulkan:
            return std::make_shared<VKRenderTarget>(window, std::static_pointer_cast<VKContext>(context.lock()));
#endif
...
#ifdef RL_USE_VULKAN
        case GraphicsAPI::Vulkan:
            return std::make_shared<VKRenderTarget>(width, height, std::static_pointer_cast<VKContext>(context.lock()));
#endif
